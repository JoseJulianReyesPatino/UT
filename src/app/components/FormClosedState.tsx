...
export function FormClosedState({ title, message, historyAction }: Readonly<FormClosedStateProps>) {
  return (
    <div className="relative mx-auto w-full max-w-6xl overflow-hidden rounded-[2rem] border border-slate-200/70 bg-gradient-to-br from-slate-50 via-white to-emerald-50 shadow-[0_24px_80px_rgba(15,23,42,0.12)] dark:border-slate-800 dark:from-slate-950 dark:via-slate-900 dark:to-emerald-950/10">
      {historyAction ? (
        <div className="absolute right-4 top-4 z-10 sm:right-6 sm:top-6">{historyAction}</div>
      ) : null}

      <div className="grid gap-0 lg:grid-cols-[minmax(16rem,0.9fr)_minmax(0,1.1fr)]">
        <div className="relative flex items-center justify-center overflow-hidden border-b border-slate-200 bg-gradient-to-br from-slate-950 via-emerald-950 to-emerald-900 p-6 sm:p-8 lg:border-b-0 lg:border-r dark:border-slate-800">
          <div className="absolute inset-0 opacity-20 [background-image:radial-gradient(circle_at_center,rgba(255,255,255,0.12)_1px,transparent_1px)] [background-size:18px_18px]" />
          ...
          <div className="relative flex w-full max-w-sm items-center justify-center rounded-[1.75rem] border border-white/10 bg-white/5 p-4 shadow-2xl backdrop-blur-sm">
            <img
              src={closedFormImage}
        ...
        <div className="flex flex-col justify-center gap-6 px-6 py-8 sm:px-8 sm:py-10 dark:bg-slate-950">

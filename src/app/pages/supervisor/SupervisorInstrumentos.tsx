...
const INSTRUMENTOS = [
  { key: "30", label: "Instrumento 30%", sublabel: "Plan Normal", form_id: 2 },
  { key: "40", label: "Instrumento 40%", sublabel: "Plan Nuevo Modelo", form_id: 3 },
  { key: "60", label: "Instrumento 60%", sublabel: "Plan Nuevo Modelo", form_id: 4 },
  { key: "70", label: "Instrumento 70%", sublabel: "Plan Normal", form_id: 5 },
] as const;
...
export default function SupervisorInstrumentos() {
  ...
  const loadDocs = useCallback(async (key: TabKey, form_id: number) => {
    setLoading((prev) => ({ ...prev, [key]: true }));
    try {
      const res = await apiFetch("/documents", { query: { form_id, per_page: 500 } }) as any;
  ...
  useEffect(() => {
    INSTRUMENTOS.forEach(({ key, form_id }) => void loadDocs(key as TabKey, form_id));
  }, [loadDocs]);
  ...
  const clearFilters = (key: TabKey) => setFilters((prev) => ({ ...prev, [key]: defaultFilter() }));
  ...
  const handlePreview = async (doc: DocRecord) => {
    setPreviewTitle(doc.uploaded_by_name ? `${doc.uploaded_by_name} — ${doc.materia ?? "Instrumento"}` : (doc.materia ?? "Instrumento"));
    ...
    <div className="flex flex-col gap-4 sm:gap-6 bg-transparent text-slate-900 dark:text-slate-100">
      ...
      <div className="shrink-0 rounded-2xl sm:rounded-3xl border border-emerald-200/30 p-4 sm:p-6 dark:border-emerald-900/25">
        ...
        <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400">
          Instrumentos de evaluación enviados por todos los docentes — solo lectura
        </p>
      </div>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <CardDescription className="text-xs sm:text-sm">
                  {isCurrentLoading
                    ? "Cargando..."
                    : `${filtered.length} de ${currentDocs.length} documento${currentDocs.length !== 1 ? "s" : ""}`}
                </CardDescription>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Button
                  variant="outline"
                  size="icon"
                  onClick={() => loadDocs(activeTab, currentInstrumento.form_id)}
            ...
            <div className="flex flex-col items-center justify-center gap-2 py-16 text-muted-foreground">
              ...
              <p className="text-sm">
                {hasActiveFilters
                  ...
                  : `No hay documentos de ${currentInstrumento.label}`}
              </p>
              {hasActiveFilters && (
                <Button variant="ghost" size="sm" onClick={() => clearFilters(activeTab)} className="text-xs mt-1">
                  Limpiar filtros
                </Button>
              )}
            </div>

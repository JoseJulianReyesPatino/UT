...
export default function PortafolioDigitalPage() {
  ...
  const isValid = useMemo(() => {
    ...
    const isUrl = /^(https?:\/\/)\S+$/i.test(formData.urlPortafolio);
    return Boolean(
      formData.plan &&
        formData.carrera &&
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-start justify-between">
        <div className="space-y-1"><h1 className="text-3xl font-bold">Portafolio Digital</h1><p className="text-muted-foreground">Envía el enlace de tu portafolio y evidencias.</p></div>
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}><SheetTrigger asChild><Button variant="outline" size="icon"><Menu className="h-5 w-5"/></Button></SheetTrigger><SheetContent side="right"><SheetHeader><SheetTitle>Seleccionar Plan</SheetTitle></SheetHeader><div className="space-y-3 mt-6"><Button variant={formData.plan === "nuevo-modelo" ? "default" : "outline"} className="w-full" onClick={() => { setFormData((c) => ({ ...c, plan: "nuevo-modelo", carrera: "", cuatrimestre: "", materia: "" })); setSheetOpen(false); }}>Plan Nuevo Modelo</Button><Button variant={formData.plan === "plan-normal" ? "default" : "outline"} className="w-full" onClick={() => { setFormData((c) => ({ ...c, plan: "plan-normal", carrera: "", cuatrimestre: "", materia: "" })); setSheetOpen(false); }}>Plan Normal</Button></div></SheetContent></Sheet>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          ...
          <div className="space-y-2"><Label>Cuatrimestre *</Label><Select value={formData.cuatrimestre} onValueChange={(v) => setFormData((c) => ({ ...c, cuatrimestre: v as Cuatrimestre, materia: "" }))} disabled={!formData.carrera}><SelectTrigger><SelectValue placeholder="Selecciona el cuatrimestre"/></SelectTrigger><SelectContent>{cuatrimestresDisponibles.map((q) => (<SelectItem key={q} value={q}>{cuatrimestresLabels[q]}</SelectItem>))}</SelectContent></Select></div>
          ...
          <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>Parcial *</Label><Select value={formData.parcial} onValueChange={(v) => setFormData((c) => ({ ...c, parcial: v }))}><SelectTrigger><SelectValue placeholder="Selecciona el parcial"/></SelectTrigger><SelectContent>{parciales.map((p) => (<SelectItem key={p} value={p}>{p}</SelectItem>))}</SelectContent></Select></div><div className="space-y-2"><Label>Grupo *</Label><Input value={formData.grupo} onChange={(e) => setFormData((c) => ({ ...c, grupo: e.target.value.toUpperCase() }))} placeholder="Ej. JTH-01" maxLength={7}/><p className="text-xs text-muted-foreground">Formato: Ej. JTH-01</p></div></div>

          <div className="space-y-2"><Label>URL del Portafolio *</Label><Input value={formData.urlPortafolio} onChange={(e)=>setFormData((c)=>({...c, urlPortafolio: e.target.value}))} placeholder="https://ejemplo.com/mi-portafolio" type="url"/></div>

          <div className="space-y-2"><Label>Descripción del portafolio *</Label><Textarea value={formData.descripcion} onChange={(e)=>setFormData((c)=>({...c, descripcion: e.target.value}))} rows={4} placeholder="Describe el contenido del portafolio"/></div>

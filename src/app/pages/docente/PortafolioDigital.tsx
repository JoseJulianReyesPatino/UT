...
export default function PortafolioDigitalPage({ deadlineInfo, onDirtyChange, layoutStyle }: { deadlineInfo?: { formattedDeadline: string; isUrgent: boolean } | null; onDirtyChange?: (dirty: boolean) => void; layoutStyle?: string }) {
  ...
  const uploadMultipleFiles = async (files: File[], basePayload: any) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const title = `${basePayload.materia || "Portafolio"} - ${cleanFileName}`.trim();
  ...
  if (isFormal) {
    return (
      <FormalFormLayout
        ...
        historialDescription="Documentos enviados anteriormente para Portafolio Digital."
          ...
          <>
            {editingDocumentId && !isMetadataOnlyEdit && (
              <div className="shrink-0 border-b border-amber-200 bg-amber-50 px-6 py-2.5 text-xs text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-100">
                Estás editando el portafolio existente. Ajusta los campos y selecciona el nuevo PDF para actualizar.
        ...
        leftColumn={
          <div className="flex flex-1 flex-col sm:overflow-y-auto p-5 border-b border-border sm:border-b-0 dark:border-slate-800">
            <p className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Subir evidencias {!isMetadataOnlyEdit && "*"}</p>
            ...
            {isMetadataOnlyEdit ? (
              ...
              <>
                <input type="file" accept=".pdf" multiple className="hidden" id="portafolio-formal-upload" onChange={handleFileChange} disabled={formData.archivos.length >= 3 || isLoadingPdf} />
                <div
                  onDrop={handleDrop}
                ...
                >
                  {isLoadingPdf ? (
                    ...
                    <label htmlFor="portafolio-formal-upload" className="block cursor-pointer space-y-3">
                    ...
                    <div className="space-y-3">
                      ...
                      {formData.archivos.length < 3 && (
                        <label htmlFor="portafolio-formal-upload" className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-background/60 py-3 text-sm text-muted-foreground transition-colors hover:border-emerald-400 hover:text-emerald-600 dark:border-slate-700 dark:hover:border-emerald-500/40">
        ...
        footerActions={
          <>
            ...
            <Button variant="success" size="sm" onClick={handleSubmit} disabled={!isValid || isSubmitting || !formAccess.canSubmit || isLoadingPdf} className="rounded-lg px-5">
              {isSubmitting ? "Enviando..." : editingDocumentId ? "Actualizar" : "Enviar portafolio"}
            </Button>
          </>
        }
        dialogs={

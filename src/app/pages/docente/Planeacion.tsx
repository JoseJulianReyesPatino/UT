...
export default function PlaneacionPage({ deadlineInfo, onDirtyChange, isTourActive, layoutStyle }: { deadlineInfo?: { formattedDeadline: string; isUrgent: boolean } | null; onDirtyChange?: (dirty: boolean) => void; isTourActive?: boolean; layoutStyle?: string }) {
  const { user } = useAuth();
  ...
  if (isFormal) {
    return (
      <FormalFormLayout
        ...
        historialDescription="Planeaciones enviadas anteriormente para revisión."
        ...
        leftColumn={
          <div className="flex flex-1 flex-col sm:overflow-y-auto p-5 border-b border-border sm:border-b-0 dark:border-slate-800">
            <p className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Planeación en PDF {!isMetadataOnlyEdit && "*"}</p>
            ...
            {isMetadataOnlyEdit ? (
              ...
              <>
                <input type="file" accept=".pdf" multiple className="hidden" id="planeacion-formal-upload" onChange={handleFileChange} disabled={formData.archivos.length >= 3 || isLoadingPdf} />
                <div
                  onDrop={handleDrop}
                ...
                >
                  {isLoadingPdf ? (
                    ...
                    <label htmlFor="planeacion-formal-upload" className="block cursor-pointer space-y-3">
                    ...
                    <div className="space-y-3">
                      ...
                      {formData.archivos.length < 3 && (
                        <label htmlFor="planeacion-formal-upload" className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-background/60 py-3 text-sm text-muted-foreground transition-colors hover:border-emerald-400 hover:text-emerald-600 dark:border-slate-700 dark:hover:border-emerald-500/40">
        ...
        rightColumn={
          <div className="flex flex-col sm:w-80 sm:shrink-0 sm:overflow-y-auto divide-y divide-border dark:divide-slate-800">
            <div className="p-5 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`rounded-lg border-2 px-3 py-2.5 text-left text-xs font-semibold transition-all ${formData.plan === "nuevo-modelo" ? "border-emerald-500 bg-emerald-50 dark:border-emerald-400 dark:bg-emerald-950/30 dark:text-white" : "border-border bg-background hover:border-emerald-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-emerald-500/30"}`}
                >
                  <span className="block">Nuevo Modelo</span>
                  <span className="block text-[10px] font-normal text-muted-foreground dark:text-slate-500">TSU e Ing.</span>
                </button>
                <button
                  type="button"
                  onClick={() => setFormData((c) => ({ ...c, plan: "plan-normal", carrera: "", cuatrimestre: "", materia: "", parcial: "", grupo: "" }))}
                  className={`rounded-lg border-2 px-3 py-2.5 text-left text-xs font-semibold transition-all ${formData.plan === "plan-normal" ? "border-emerald-500 bg-emerald-50 dark:border-emerald-400 dark:bg-emerald-950/30 dark:text-white" : "border-border bg-background hover:border-emerald-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-emerald-500/30"}`}
                >
                  <span className="block">Plan Normal</span>
                  <span className="block text-[10px] font-normal text-muted-foreground dark:text-slate-500">Ingenierías</span>
                </button>
              </div>
            </div>
            <div className="p-5 space-y-2">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Carrera *</p>
              <Select value={formData.carrera} onValueChange={(value) => setFormData((current) => ({ ...current, carrera: value, cuatrimestre: "", materia: "", parcial: "", grupo: "" }))} disabled={!formData.plan}>
                <SelectTrigger className="rounded-lg text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-white"><SelectValue placeholder={formData.plan ? "Selecciona la carrera" : "Elige plan primero"} /></SelectTrigger>
            ...
            <div className="p-5 space-y-2">
              ...
              <Select value={formData.cuatrimestre} onValueChange={(value) => setFormData((current) => ({ ...current, cuatrimestre: value as Cuatrimestre, materia: "", parcial: "", grupo: "" }))} disabled={!formData.carrera}>
                ...
                <SelectContent className="dark:border-slate-700 dark:bg-slate-900" position="popper" avoidCollisions={false}>
                  {cuatrimestresDisponibles.map((value) => (<SelectItem key={value} value={value} className="dark:text-white dark:hover:bg-slate-800">{cuatrimestresLabels[value as keyof typeof cuatrimestresLabels]}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <div className="p-5 space-y-2">
            ...
            <div className="p-5 space-y-2">
              ...
              <Textarea value={formData.nota} onChange={(e) => setFormData((current) => ({ ...current, nota: e.target.value }))} placeholder="Opcional" className="min-h-[6rem] resize-none rounded-lg text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder:text-slate-500" />
            </div>
            <div className="p-5 space-y-2">

...
export default function Instrumento60Page({ deadlineInfo, onDirtyChange, layoutStyle }: { deadlineInfo?: { formattedDeadline: string; isUrgent: boolean } | null; onDirtyChange?: (dirty: boolean) => void; layoutStyle?: string }) {
  const { user } = useAuth();
        ...
        leftColumn={
          <div className="flex flex-1 flex-col sm:overflow-y-auto p-5 border-b border-border sm:border-b-0 dark:border-slate-800">
            ...
            {isMetadataOnlyEdit ? (
              ...
              <>
                <input type="file" accept=".pdf" multiple className="hidden" id="instrumento-60-formal-upload" onChange={handleFileChange} disabled={formData.archivos.length >= 3 || isLoadingPdf} />
                <div
                  onDrop={handleDrop}
                ...
                >
                  {isLoadingPdf ? (
                    ...
                    <label htmlFor="instrumento-60-formal-upload" className="block cursor-pointer space-y-3">
                    ...
                    <div className="space-y-3">
                      ...
                      {formData.archivos.length < 3 && (
                        <label htmlFor="instrumento-60-formal-upload" className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-background/60 py-3 text-sm text-muted-foreground transition-colors hover:border-emerald-400 hover:text-emerald-600 dark:border-slate-700 dark:hover:border-emerald-500/40">

...
export default function ActaFinalPage({ deadlineInfo, onDirtyChange, layoutStyle }: { deadlineInfo?: { formattedDeadline: string; isUrgent: boolean } | null; onDirtyChange?: (dirty: boolean) => void; layoutStyle?: string }) {
  const isFormal = layoutStyle === "formal";
  const { user } = useAuth();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const formAccess = useFormAccess(ACTA_FINAL_FORM_ID);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const res = await apiFetch("/documents", { query: { uploaded_by: user.id, form_id: ACTA_FINAL_FORM_ID, per_page: 50 } });
        if (cancelled) return;
  ...
  const uploadMultipleFiles = async (files: File[], basePayload: any) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const title = `${basePayload.materia || "Acta Final"} - ${cleanFileName}`.trim();
  ...
  if (isFormal) {
    return (
      <FormalFormLayout
        ...
        historialDescription="Documentos enviados anteriormente para Acta Final."
          ...
          <>
            {editingDocumentId && !isMetadataOnlyEdit && (
              <div className="shrink-0 border-b border-amber-200 bg-amber-50 px-6 py-2.5 text-xs text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-100">
                Estás editando el acta existente. Ajusta los campos y selecciona el nuevo PDF para actualizar.
        ...
        leftColumn={
          <div className="flex flex-1 flex-col sm:overflow-y-auto p-5 border-b border-border sm:border-b-0 dark:border-slate-800">
            ...
            {isMetadataOnlyEdit ? (
              ...
              <>
                <input type="file" accept=".pdf" multiple className="hidden" id="acta-final-formal-upload" onChange={handleFileChange} disabled={formData.archivos.length >= 3 || isLoadingPdf} />
                <div
                  onDrop={handleDrop}
                ...
                >
                  {isLoadingPdf ? (
                    ...
                    <label htmlFor="acta-final-formal-upload" className="block cursor-pointer space-y-3">
                    ...
                    <div className="space-y-3">
                      ...
                      {formData.archivos.length < 3 && (
                        <label htmlFor="acta-final-formal-upload" className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-background/60 py-3 text-sm text-muted-foreground transition-colors hover:border-emerald-400 hover:text-emerald-600 dark:border-slate-700 dark:hover:border-emerald-500/40">
          ...
          <div className="flex flex-col sm:w-80 sm:shrink-0 sm:overflow-y-auto divide-y divide-border dark:divide-slate-800">
            <div className="p-5 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => setFormData((c) => ({ ...c, plan: "nuevo-modelo", carrera: "", cuatrimestre: "", materia: "", grupo: "" }))}
                ...
                <button
                  type="button"
                  onClick={() => setFormData((c) => ({ ...c, plan: "plan-normal", carrera: "", cuatrimestre: "", materia: "", grupo: "" }))}
                  className={`rounded-lg border px-3 py-2 text-xs font-medium transition-colors ${
            ...
            <div className="p-5 space-y-2">
              ...
              <Select value={formData.carrera} onValueChange={(v) => setFormData((c) => ({ ...c, carrera: v, cuatrimestre: "", materia: "", grupo: "" }))} disabled={!formData.plan}>
            ...
            <div className="p-5 space-y-2">
              ...
              <Select value={formData.cuatrimestre} onValueChange={(v) => setFormData((c) => ({ ...c, cuatrimestre: v as Cuatrimestre, materia: "", grupo: "" }))} disabled={!formData.carrera}>
                <SelectTrigger className="rounded-lg text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-white">
            ...
            <div className="p-5 space-y-2">
              ...
              <Select value={formData.materia} onValueChange={(v) => setFormData((c) => ({ ...c, materia: v, grupo: "" }))} disabled={!formData.cuatrimestre}>
                <SelectTrigger className="rounded-lg text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-white">
            ...
            <div className="p-5 space-y-2">
              ...
              <Select value={formData.grupo} onValueChange={(v) => setFormData((c) => ({ ...c, grupo: v }))} disabled={!formData.materia}>
                <SelectTrigger className="rounded-lg text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-white">
        ...
        footerActions={
          <>
            ...
            <Button variant="success" size="sm" onClick={handleSubmit} disabled={!isValid || isSubmitting || !formAccess.canSubmit || isLoadingPdf} className="rounded-lg px-5">
              {isSubmitting ? "Enviando..." : editingDocumentId ? "Actualizar" : "Enviar acta"}
            </Button>
          </>
        }
        dialogs={
    ...
    <div className="max-w-4xl mx-auto space-y-1" ref={formRef}>
      ...
      <div className="space-y-1.5 pt-1">
        ...
        <p className="text-white/90 drop-shadow-sm dark:text-slate-400">
          Envía el acta oficial de calificaciones finales del cuatrimestre.
        </p>
      </div>
        ...
        <CardContent className="relative space-y-6 p-6 pt-5 sm:p-8 sm:pt-6">
          ...
          {editingDocumentId && !isMetadataOnlyEdit && (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100">
              Estás editando el acta existente. Ajusta los campos y selecciona el nuevo archivo PDF para actualizar.
              {isLoadingPdf && (
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              {isMetadataOnlyEdit ? (
                ...
                <p className="text-sm text-muted-foreground dark:text-slate-400">Adjunta el PDF del acta de hasta 15 MB. Puedes cargar hasta tres archivos en total.</p>
              )}
          ...
          {formAccess.isExpired && (
            <div className="flex items-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 mb-2 text-sm font-medium text-red-800 dark:border-red-900/40 dark:bg-red-950/30 dark:text-red-300">
              <span>Formulario cerrado &mdash; el plazo de envío ha vencido. Solo puedes consultar tu historial.</span>
            </div>
          )}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {resubmitFile ? (
                ...
                <label htmlFor="acta-final-resubmit-upload" className="flex cursor-pointer flex-col items-center gap-2 rounded-xl border-2 border-dashed border-border px-4 py-6 text-center transition hover:border-emerald-400 hover:bg-emerald-50/30 dark:border-slate-700 dark:hover:border-emerald-500/40 dark:hover:bg-emerald-950/10">

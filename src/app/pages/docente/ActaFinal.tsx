...
export default function ActaFinalPage({ deadlineInfo }: { deadlineInfo?: { formattedDeadline: string; isUrgent: boolean } | null }) {
  const { user } = useAuth();
    ...
    <div className="max-w-4xl mx-auto space-y-1" ref={formRef}>
      ...
      <Card className="overflow-hidden border-border/70 bg-card shadow-sm dark:border-border/70 dark:bg-card dark:border-slate-800/70 dark:bg-slate-950/60">
        <CardContent className="relative space-y-6 p-6 pt-5 sm:p-8 sm:pt-6">
          ...
          {/* Formulario cerrado - expirado */}
          {formAccess.isExpired && (
            <div className="flex items-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-800 dark:border-red-900/40 dark:bg-red-950/30 dark:text-red-300">

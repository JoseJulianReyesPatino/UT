...
export default function RemedialPage({ deadlineInfo, onDirtyChange, layoutStyle }: { deadlineInfo?: { formattedDeadline: string; isUrgent: boolean } | null; onDirtyChange?: (dirty: boolean) => void; layoutStyle?: string }) {
  const { user } = useAuth();
  ...
  const populateFormForEditBatch = async (documents: any[]) => {
    ...
    const rawCuatrimestre = asText(main.cuatrimestre);
    const titleParts = extractTitleParts(main.title ?? main.file_path ?? "");
    const resolvedCuatrimestre = allowedCuatrimestres.has(rawCuatrimestre)
      ? rawCuatrimestre
      : extractCuatrimestreFromGroupCode(main.group_code ?? main.group ?? "", careerCode);
    ...
    setFormData({
      ...
      materia: normalizeText(main.materia, titleParts.materia),
      parcial: normalizeParcialForForm(main.parcial, titleParts.parcial),
      grupo: formatGroupCode(main.group_code ?? main.group ?? ""),
      archivos: [],
  ...
  if (isFormal) {
    return (
      <FormalFormLayout
        ...
        historialDescription="Documentos enviados anteriormente para Remedial."
          ...
          <>
            {editingDocumentId && !isMetadataOnlyEdit && (
              <div className="shrink-0 border-b border-amber-200 bg-amber-50 px-6 py-2.5 text-xs text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-100">
                Estás editando el remedial existente. Ajusta los campos y selecciona el nuevo PDF para actualizar.
        ...
        leftColumn={
          <div className="flex flex-1 flex-col sm:overflow-y-auto p-5 border-b border-border sm:border-b-0 dark:border-slate-800">
            ...
            {isMetadataOnlyEdit ? (
              ...
              <>
                <input type="file" accept=".pdf" multiple className="hidden" id="remedial-formal-upload" onChange={handleFileChange} disabled={formData.archivos.length >= 3 || isLoadingPdf} />
                <div
                  onDrop={handleDrop}
                ...
                >
                  {isLoadingPdf ? (
                    ...
                    <label htmlFor="remedial-formal-upload" className="block cursor-pointer space-y-3">
                    ...
                    <div className="space-y-3">
                      ...
                      {formData.archivos.length < 3 && (
                        <label htmlFor="remedial-formal-upload" className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-background/60 py-3 text-sm text-muted-foreground transition-colors hover:border-emerald-400 hover:text-emerald-600 dark:border-slate-700 dark:hover:border-emerald-500/40">
              ...
              <DialogContent className={`max-w-[calc(100vw-2rem)] ${resubmitFile ? "sm:max-w-2xl" : "sm:max-w-md"} dark:border-slate-800/70 dark:bg-slate-950/90 dark:backdrop-blur-md`}>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium dark:text-white">Nuevo archivo PDF *</label>
                    <input type="file" accept=".pdf" id="resubmit-pdf-upload-formal" className="hidden" onChange={(e) => {

...
const TUTORIAS_FORM_ID_MAP: Record<DocumentoTutorias, string> = {
  "carga-academica": "carga-academica",
  "reporte-bajas": "reporte-bajas",
  "concentrado-asesorias": "concentrado-asesorias",
  "acta-asistencia-grupal": "acta-asistencia-grupal",
...
export default function TutoriasPage(props: Readonly<TutoriasPageProps> = {}) {
  const { initialType = null, onNavigateHome, deadlineInfo, layoutStyle } = props;
  ...
  useEffect(() => {
    ...
    preloadForms().then((forms) => {
      ...
      for (const type of documentTypes) {
        const formIdStr = TUTORIAS_FORM_ID_MAP[type.id];
  ...
  useEffect(() => {
    ...
    const formIdStr = TUTORIAS_FORM_ID_MAP[selectedType];
    preloadForms().then((forms) => {
      const match = forms.find((f: any) => String(f.form_code).replace(/_/g, "-") === formIdStr);
    ...
    return (
      <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm dark:border-slate-800 dark:bg-slate-950">
        ...
        <div className="border-b border-border bg-card px-6 py-4 dark:border-slate-800 dark:bg-slate-950/80">
          <h1 className="text-xl font-semibold tracking-tight text-slate-900 dark:text-white">Tutorías</h1>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  {/* Fecha límite visible solo en móvil */}
                  {dl && (

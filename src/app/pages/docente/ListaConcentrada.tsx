  ...
  const uploadMultipleFiles = async (files: File[], basePayload: any) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const payload = {
        ...
        file_type: file.type,
        file_size: file.size,
        title: `${basePayload.materia || "Lista Concentrada"} - ${basePayload.parcial || ""} - ${cleanFileName}`.trim(),
      };
                        ...
                        <div key={h.id} className="rounded-lg border border-border bg-card p-4 shadow-sm dark:bg-slate-900">
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">{h.materia ?? "Lista Concentrada"}</p>
                            </div>
        ...
        <CardHeader>
          ...
          {editingDocumentId && (
            <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100">
              Estás editando la lista concentrada existente #{editingDocumentId}. Ajusta los campos y selecciona el nuevo archivo PDF para actualizar.
            </div>
          )}
        </CardHeader>

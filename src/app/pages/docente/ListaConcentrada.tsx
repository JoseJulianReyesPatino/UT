...
export default function ListaConcentradaPage() {
  ...
  const uploadMultipleFiles = async (files: File[], basePayload: any) => {
    ...
    for (const file of files) {
      ...
      const title = `${basePayload.materia || "Lista Concentrada"} - ${basePayload.parcial || ""} - ${cleanFileName}`.trim();
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <div className="rounded-3xl border border-dashed border-border bg-background/60 p-6 text-center transition-colors hover:border-primary/50 hover:bg-primary/5 dark:border-slate-700 dark:bg-slate-900/30 dark:hover:border-emerald-500/40">
                <input type="file" accept=".pdf" multiple className="hidden" id="lista-concentrada-upload" onChange={handleFileChange} disabled={formData.archivos.length >= 3} />
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <Textarea value={formData.nota} onChange={(e) => setFormData((current) => ({ ...current, nota: e.target.value }))} placeholder="Agrega una nota para revisión" className="min-h-[9rem] rounded-2xl dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder:text-slate-500" />
            </div>
          </div>

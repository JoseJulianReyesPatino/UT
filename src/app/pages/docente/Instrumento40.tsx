...
export default function Instrumento40Page() {
  ...
  const uploadMultipleFiles = async (files: File[], basePayload: any) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const payload = {
        ...
        file_type: file.type,
        file_size: file.size,
        title: `${basePayload.materia || "Instrumento 40%"} - ${basePayload.parcial || ""} - ${cleanFileName}`.trim(),
      };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6" ref={formRef}>
                      ...
                      <div key={h.id} className="rounded-lg border border-border bg-card p-4 shadow-sm dark:bg-slate-900">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{h.materia ?? "Instrumento 40%"}</p>
                          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Formulario Instrumento 40%</CardTitle>

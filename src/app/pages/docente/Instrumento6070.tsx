...
interface Instrumento6070FormData {
  plan: Plan | "";
  carrera: string;
...
const initialFormData: Instrumento6070FormData = {
  plan: "",
  carrera: "",
  cuatrimestre: "",
  materia: "",
...
export default function Instrumento6070Page() {
  const entryBase = "motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-1 motion-safe:duration-200 motion-reduce:animate-none";
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [formData, setFormData] = useState<Instrumento6070FormData>(initialFormData);
  ...
  const handleSubmit = async () => {
    ...
    setIsSubmitting(true); await new Promise((r) => setTimeout(r, 1000)); toast.success("Instrumento 60/70 enviado correctamente"); setIsSubmitting(false); resetForm();
  };

  return (
    <div className={`max-w-4xl mx-auto space-y-6 ${entryBase}`}>
      <div className={`flex items-start justify-between ${entryBase} motion-safe:delay-75`}>
        <div className="space-y-1 transition-transform duration-200">
          <h1 className="text-3xl font-bold">Instrumento 60/70</h1>
          <p className="text-muted-foreground">Captura y envía tu instrumento de evaluación 60/70.</p>
        </div>
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          <SheetTrigger asChild><Button variant="outline" size="icon" className="transition-all duration-200 hover:scale-105 active:scale-95"><Menu className="h-5 w-5"/></Button></SheetTrigger>
      ...
      <div className={`flex items-center justify-between ${entryBase} p-3 bg-emerald-50 dark:bg-emerald-900/10 border-l-4 border-emerald-300 dark:border-emerald-300 rounded-md`}>
      ...
      <Card className={`${entryBase} motion-safe:delay-100 transition-all duration-300 hover:shadow-md`}>
        <CardHeader className={`${entryBase} motion-safe:delay-150`}><CardTitle>Formulario Instrumento 60/70</CardTitle><CardDescription>Los campos marcados con * son obligatorios.</CardDescription></CardHeader>
        <CardContent className="space-y-5">
          ...
          <div className={`space-y-2 ${entryBase} motion-safe:delay-200`}><Label>Carrera *</Label>
          ...
          <div className={`space-y-2 ${entryBase} motion-safe:delay-200`}><Label>Cuatrimestre *</Label>
            <Select value={formData.cuatrimestre} onValueChange={(v) => setFormData((c) => ({ ...c, cuatrimestre: v as Cuatrimestre, materia: "" }))} disabled={!formData.carrera}>
              ...
              <SelectContent>{cuatrimestresDisponibles.map((q) => (<SelectItem key={q} value={q}>{cuatrimestresLabels[q as keyof typeof cuatrimestresLabels]}</SelectItem>))}</SelectContent>
            </Select>
          </div>

          <div className={`space-y-2 ${entryBase} motion-safe:delay-200`}><Label>Materia *</Label>
            <Select value={formData.materia} onValueChange={(v) => setFormData((c) => ({ ...c, materia: v }))} disabled={!formData.cuatrimestre}>
              ...
              <SelectContent>{materiasDisponibles.map((m, i) => (<SelectItem key={`${m.nombre}-${i}`} value={m.nombre}>{m.nombre}</SelectItem>))}</SelectContent>
            </Select>
          </div>

          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2"><Label>Parcial *</Label>
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">Adjuntar el documento en formato PDF, con un límite de 2 MB por archivo. En caso de ser necesario, se permite la carga simultánea de hasta tres archivos.</p>
            <div className={`border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-success/50 transition-all duration-200 hover:-translate-y-0.5 ${entryBase} motion-safe:delay-200`}>
              <input type="file" accept=".pdf" multiple className="hidden" id="instrumento-60-upload" onChange={handleFileChange} disabled={formData.archivos.length>=3}/>
              <label htmlFor="instrumento-60-upload" className="cursor-pointer block space-y-2"><Upload className="h-8 w-8 mx-auto text-muted-foreground transition-transform duration-200 hover:scale-105"/><p className="text-sm font-medium">{getArchivosLabel()}</p><p className="text-xs text-muted-foreground">{getEspaciosLabel()}</p></label>
            </div>

            {formData.archivos.length>0 && (<div className="space-y-2">{formData.archivos.map((archivo, index)=>(<div key={`${archivo.name}-${archivo.size}-${index}`} className="p-3 bg-success/10 border border-success/20 rounded-lg flex items-center justify-between transition-all duration-200 hover:shadow-sm hover:-translate-y-0.5 motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-1 motion-safe:duration-200"><div className="flex items-center gap-2 text-sm flex-1"><FileText className="h-4 w-4 text-success"/><span className="font-medium">{archivo.name}</span></div><Button variant="ghost" size="sm" onClick={()=>removeFile(index)} className="h-6 w-6 p-0 transition-transform duration-200 hover:scale-110"><X className="h-4 w-4"/></Button></div>))}</div>)}
          </div>

          {formData.archivos.length>0 && (<div className="space-y-2 motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-1 motion-safe:duration-200 motion-safe:delay-200">{formData.archivos.map((archivo,index)=>(<PdfPreview key={`preview-${archivo.name}-${archivo.size}-${index}`} file={archivo} title={`Vista previa - ${archivo.name}`}/>))}</div>)}
          ...
          <div className={`flex gap-3 pt-4 border-t ${entryBase} motion-safe:delay-200`}><Button variant="outline" onClick={resetForm} disabled={isSubmitting} className="transition-all duration-200 hover:-translate-y-0.5">Limpiar</Button><Button variant="success" onClick={handleSubmit} disabled={!isValid||isSubmitting} className="transition-all duration-200 hover:-translate-y-0.5">{isSubmitting?"Enviando...":"Enviar instrumento"}</Button></div>
        </CardContent>
      </Card>
    </div>
  );
